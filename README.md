...
- `nnmodel.py`: it contains the `NNModel` python class. It is derived from the `torch.nn.Module` class and it incapsulates the neural network model to be studied using the sampling algorithms `PLSampler` and `HMCSampler`.
- `ffn/`: the `ffn.py` python script contains the implementation of a standard three-layer feedforward neural network studied in [PAPER](http://arxiv.org/abs/2603.15367) as the python class `FeedforwardNet`.
...
- `custom_generator.py`: it implements a custom pytorch random-number generator used in the `PLSampler` and `HMCSampler`, named `CustomGenerator`.

### utils/ <a name="utils"></a>
- `general.py`: it contains various functions for input file reading and directory management.
