...
# CE GIUSTA
def choose_cost(name:str, device, **kwargs):
	...
	if name == 'MSE':
		...
		# y = posizione nel dizionario da prendere, aminoacido maskerato
		# fx = lunghezza *dizionario lungh
		# ripeuti n volte per le n sequenze
		...
		#if kwargs['mean']: Cost = lambda fx, masks: sum(
		#      -k*torch.log(torch.nn.functional.softmax(fx, dim=1)[i])[torch.tensor([x[0] for x in mask]).to(device), torch.tensor([x[1] for x in mask]).to(device)].mean()
		#      for i, mask in enumerate(masks)) / len(masks)
		#if kwargs['mean']: Cost = lambda fx, masks: new_cost(fx, masks)
		#else: Cost = lambda fx, masks: sum(
		#    -k*torch.log(torch.nn.functional.softmax(fx, dim=1)[i])[torch.tensor([x[0] for x in mask]).to(device), torch.tensor([x[1] for x in mask]).to(device)])
...
# accuracy_CE dovrebbe essere giusta
def choose_metric(name:str, device, **kwargs):
