...
def load_data(filedir, which):
	assert which in ["train", "test"], f"load(): unexpected value of the inputted 'which' variable ({which}). Allowed values: 'train', 'test'."
	x, y = torch.load(f"{CWD}/mnist_{which}.pt", weights_only=True)
	return x, y


# Generate three datasets, i.e. training, validation and test
def load_datasets(filedir, P_train, P_val, P_test, flatten=False, seed_tvs=0, seed_test=1, device="cpu"):
