...
class HMCSampler():
	...
	def sample(
		...
		for idx, pars in enumerate(pars_list):
			...
			for move in range(data["move"]+1, pars["tot_moves"]):
			...
			if idx+1 < len(pars_list):
				_ = self._extract_and_integrate(pars)
			else:
				self._save_log(data, settings)
				self._print_status(data)
	...
	def _compute_observables(self, lamda, gamma, bss, backward=True, extra=False):
		...
		return {
				"loss": cost + reg.detach().item(),
				"cost": cost,
				"mod2": mod2.detach().item(),
				"d2": d2.detach().item(), 
				"metric": metric if extra else None,
		}
	...
	def _print_pars(self, pars, settings, idx):
		...
		lines.append(f'# integration time step:      {pars["dt"]:.3f}')
