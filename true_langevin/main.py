...
from samplers.tl_sampler import TLSampler
...
from models.transformer import Transformer, TModel, NNModel
...
def get_exists(extra):
	results_files = ['data.dat', 'pars.txt', 'generator.npy', 'log.pt']
	weights_files = [f"{name}_0.pt" for name in ('weights', 'momenta', 'pars')]
	exists = all([rf in os.listdir(extra['results_dir']) for rf in results_files] + [[wf in os.listdir(extra['weights_dir']) for wf in weights_files]])
	return exists
...
def restart(pars, extra):
	...
	for fn in ['data']:
		stuff = load_stuff(f'{extra["results_dir"]}/{fn}.dat')
		stuff = stuff.loc[np.array(stuff['move']) <= last_move, :].reset_index(drop=True)
		with open(f'{extra["results_dir"]}/{fn}.dat', 'w') as f:
...
def reset(pars, extra):
	...
	for d in [extra['results_dir'], extra['weights_dir']]:
...
def summary(spars, extra, model, title):
	...
	lines.append(f'# mass:            {spars["m"]:.0f}')
